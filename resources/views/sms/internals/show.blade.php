...
<div class="row">
		...
		<h1>Internal Marks</h1>
			...
			@foreach($student_subjects as $student_subject)
				<tr>
					...
					<td><a href="{{route('sms.internals.edit',$student_subject->id)}}" class="btn btn-info btn-xs">Edit</a></td>
					<td>
					@if(isset($student_subject->internal['id']))
						{!! Form::open(['route'=>['sms.internals.destroy',$student_subject->internal['id']],'method'=>'delete']) !!}
	...
	<div class="col-md-2">
		...
		{{ $student->name }}
			...
			<div class="row">
				<div class="col-md-12">
				{{Html::linkRoute('sms.students.index','<<All Students',[],['class'=>'btn btn-primary btn-block btn-h1-spacing'])}}
				</div>
			</div>
			<div class="row">
				<div class="col-md-12">
